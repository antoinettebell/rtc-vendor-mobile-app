...
const ComboItemsActionSheet = ({
  ...
  const handleItemSelect = (item) => {
    setInternalSelectedMenus((prev) => {
      ...
      if (isSelected) {
        const newSelection = prev.filter((selected) => selected._id !== item._id);
        ...
        {internalSelectedMenus.length > 0 && (
          <View style={styles.selectedItemsContainer}>
            <Text style={styles.selectedItemsTitle}>
              Selected Items ({internalSelectedMenus.length}{limit !== undefined ? `/${limit}` : ''})
            </Text>
...
export default ComboItemsActionSheet;
