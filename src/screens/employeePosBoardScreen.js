...
const EmployeePosBoardScreen = ({ navigation, route }) => {
  ...
  const openOptions = (item, options = {}) => {
    ...
    setSelectedDiscountFlavors(addAnother ? [] : existing?.selectedDiscountFlavors || []);
    setSelectedDiscountToppings(addAnother ? [] : existing?.selectedDiscountToppings || []);
    setSelectedSubItems(addAnother ? [] : existing?.selectedSubItems || []);
