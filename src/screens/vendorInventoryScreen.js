...
const inventoryEntries = (forms = []) => forms
  ...
  .flatMap((form) => (form.inventory_items || []).map((item) => ({
    ...
    inventory_status: form.status === "ARCHIVED" || item.lifecycle_status === "ARCHIVED" ? "Archived" : "Active",
  ...
  if (selected) {
    ...
    return <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}><Text style={styles.title}>{selected.item_name || "Add Inventory Item"}</Text><Text style={styles.meta}>{archived ? "Archived Inventory" : selected.truck || "Choose food truck"}</Text></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  activeBadge: { backgroundColor: "#DCFCE7", borderRadius: 12, color: "#166534", fontSize: 11, fontWeight: "700", overflow: "hidden", paddingHorizontal: 8, paddingVertical: 4 }, archivedBadge: { backgroundColor: "#E2E8F0", borderRadius: 12, color: "#475569", fontSize: 11, fontWeight: "700", overflow: "hidden", paddingHorizontal: 8, paddingVertical: 4 },
